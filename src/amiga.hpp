...
enum class Mode : unsigned char {
    ...
    // BC3 (DXT5) RGBA block-compressed texture. 4×4 blocks, 8 bpp.
    // BC1 RGB color block + BC4-style 8-bit alpha block. KTX2
    // VK_FORMAT_BC3_*_BLOCK or DDS FourCC "DXT5".
    bc3,

    // BC7 RGBA block-compressed texture. 4×4 blocks, 8 bpp. 8 sub-modes
    // (currently emits Mode 6 only). KTX2 container;
    // VK_FORMAT_BC7_UNORM_BLOCK / _SRGB_BLOCK.
    bc7,
...
constexpr ModeParams get_mode_params(Mode mode) noexcept {
    ...
    case Mode::astc_10x6:
    case Mode::astc_10x8:
    case Mode::astc_10x10:
    case Mode::astc_12x10:
    case Mode::astc_12x12:
    case Mode::bc2:
    case Mode::bc3:
    case Mode::bc7:
        return {0, 0, 8, 0, false, false, false, false, 1, 1, 1.0f};
...
constexpr bool is_cga(Mode mode) noexcept {
    return mode == Mode::cga_320 || mode == Mode::cga_640 ||
           mode == Mode::cga_composite_hires || mode == Mode::cga_text80x100 ||
           mode == Mode::cga_text80x50 || mode == Mode::cga_text80x25 ||
...
constexpr bool is_astc(Mode mode) noexcept {
    return mode == Mode::astc_4x4 || mode == Mode::astc_5x4 ||
           ...
           mode == Mode::astc_6x6 || mode == Mode::astc_8x5 ||
           mode == Mode::astc_8x6 || mode == Mode::astc_8x8 ||
           mode == Mode::astc_10x5 || mode == Mode::astc_10x6 ||
           mode == Mode::astc_10x8 || mode == Mode::astc_10x10 ||
           mode == Mode::astc_12x10 || mode == Mode::astc_12x12;
}

// BC7 RGBA block compression — same bypass treatment as etc2.
constexpr bool is_bc7(Mode mode) noexcept { return mode == Mode::bc7; }

// Any GPU block-compression mode (ETC2, BC1, BC7, future ASTC). Convenient
...
constexpr bool is_composite(Mode mode) noexcept {
    return mode == Mode::cga_composite_hires;
}
