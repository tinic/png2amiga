...
Result<Palette> gpu_restart_quantize(const Image& image,
    ...
    if (!png2amiga_metal_available_c()) {
        return std::unexpected{Error{ErrorCode::unsupported_mode, "Metal device unavailable"}};
    }
    ...
    int rc = png2amiga_quantize_metal_c(
        rgb.data(), n, max_colors, restarts, iterations, seed, pal_rgb.data(), &out_size);
    if (rc != 0) {
    ...
    for (std::size_t k = 0; k < out_size; ++k) {
        out.colors.push_back(Color3f{pal_rgb[k * 3 + 0], pal_rgb[k * 3 + 1], pal_rgb[k * 3 + 2]});
    }
    return out;
}
