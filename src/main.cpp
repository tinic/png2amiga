...
void print_usage() {
    ...
    std::println(stderr,
        ...
        "    Commodore 64 / VIC-II (16-colour fixed palette, see --c64-palette):\n"
        "         c64-multicolor    (160x200 logical, 4×8 cells, 4 colours/cell)\n"
        "         c64-hires         (320x200, 8×8 cells, 2 colours/cell)\n"
        "         c64-fli           (160x200, FLI multicolor, per-row screen swap)\n"
        "         c64-afli          (320x200, hires FLI, per-row screen swap)\n"
        "         c64-petscii       (40x25 text mode, glyph match against ROM font)\n"
        "         c64-charset-hires       (8x8-cell freeform via --width/--height,\n"
        "                                  ≤ --tile-budget unique 8-byte glyphs)\n"
        "         c64-charset-multicolor  (4x8-cell freeform; 1:1 source pixels —\n"
        "                                  encoder halves width internally)\n"
        "                            FLI / AFLI: cells 0..2 always show $D021 (bug).\n"
        "                            Output: .png preview, .raw bytes,\n"
        "                                    .h header (charset modes),\n"
        "                                    .prg displayer (Koala / Art Studio /\n"
        "                                    FLI / AFLI / PETSCII), or\n"
        "                                    .koa / .hir raw image-only formats.\n"
